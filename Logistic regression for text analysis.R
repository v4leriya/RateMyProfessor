...
#subset the data that contains non-missing observations for the binary would_take_agains
...
train1=subset(rmp_log, split==TRUE)
test1=subset(rmp_log, split==FALSE)
...
#split the data into training and testing 70/30
split=sample.split(labeledComments$sentiment, SplitRatio=0.7)
train2=subset(labeledComments, split==TRUE)
test2=subset(labeledComments, split==FALSE)
